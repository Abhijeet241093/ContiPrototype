...
let model =null
try {
  model = await loadGraphModel('C:\Users\Abhijeet\Desktop\Yes\chung-ang-prototype-main\microservices\nodejs\src\routes\model.json');
}
catch {
   
}
router.post("/ss1", async (req, res) => {
  try {
    const { key } = req.body;
    if (!model) {
      model = await loadGraphModel('C:\Users\Abhijeet\Desktop\Yes\chung-ang-prototype-main\microservices\nodejs\src\routes\model.json');
...
router.post("/", async (req, res) => {
  try {
    const { key } = req.body;
    if (!model) {
      model = await tf.loadGraphModel('C:\Users\Abhijeet\Desktop\Yes\chung-ang-prototype-main\microservices\nodejs\src\routes\model.json');
    }
    let url = await createPublicUrl(key, 3000)
    let data = await detectFrame(url, model);
    res.status(200).json(new ResponseData(true, data, "Success"));
